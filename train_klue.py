...
class KoGPT2Chat(LightningModule):
    ...
    def test_epoch_end(self, test_step_outputs):
        ...
        with open('etc/ALL_RESULT13K.txt', 'a') as f:
            if args.bk:
    ...
    def train_dataloader(self):
        data = pd.read_csv(self.DS)
        train_set = CharDataset(data, max_len=self.hparams.max_len)
    ...
    def test_dataloader(self):
        if args.k3:
            data = pd.read_csv('GKSL/GKSL3k(final)_mix.csv')
        elif args.bk:
            data = pd.read_csv('MY_DATA/gloss_from_book.csv')
        test_set = CharDataset(data, max_len=self.hparams.max_len)
        return DataLoader(test_set, batch_size=self.hparams.batch_size, num_workers=2, collate_fn=self._collate_fn)

    def chat(self):

        with torch.no_grad():
            if not args.t2g: q_str, a_str = 'gloss  >>> ', 'spoken >>>'
            ...
            while True:
                ...
                a = ''
                while True:
                    input_ids = torch.LongTensor(self.TKNZR.encode(U_TKN + q + EOS + '>' + S_TKN + a)).unsqueeze(dim=0)
                    pred = self(input_ids)
    ...
    def get_bleu_score(self):
        df = pd.read_csv('GKSL/GKSL3k_train.csv')
        ...
        with torch.no_grad():
            for i in range(len(df)):
                ...
                if flag: continue
                a = a.replace('▁', ' ')
                gt = tagger.parse(gt.strip()).split()
                a = tagger.parse(a.strip()).split()
                score1 = sentence_bleu([gt], a, weights=(1, 0, 0, 0))
                score2 = sentence_bleu([gt], a, weights=(0, 1, 0, 0))
                score3 = sentence_bleu([gt], a, weights=(0, 0, 1, 0))
                score4 = sentence_bleu([gt], a, weights=(0, 0, 0, 1))
                scoret = sentence_bleu([gt], a)
                ...
                print(f"{i} / {len(df)}")
        BLEU1_AVG, BLEU2_AVG, BLEU3_AVG, BLEU4_AVG, BLEUT_AVG \
            = np.mean(SCORE1), np.mean(SCORE2), np.mean(SCORE3), np.mean(SCORE4), np.mean(SCORET)
        print(f"{args.model_params}  === BLEU score: {BLEU1_AVG:.3f} {BLEU2_AVG:.3f} {BLEU3_AVG:.3f} {BLEU4_AVG:.3f} {BLEUT_AVG:.3f}")

        with open('etc/bleu_score.txt', 'a') as f:
            if args.bk:
                f.write(args.model_params + '  =bk=  '
                        + f'{BLEU1_AVG:.3f}, {BLEU2_AVG:.3f}, {BLEU3_AVG:.3f}, {BLEU4_AVG:.3f}, {BLEUT_AVG:.3f}' + '\n')
            elif args.k3:
                f.write(args.model_params + '  =3k=  '
                        + f'{BLEU1_AVG:.3f}, {BLEU2_AVG:.3f}, {BLEU3_AVG:.3f}, {BLEU4_AVG:.3f}, {BLEUT_AVG:.3f}' + '\n')
...
def configure_parser():
    ...
    parser.add_argument('--bleu', action='store_true', default=False, help='get bleu score')
...
def measure_model(model_params):
    model = KoGPT2Chat.load_from_checkpoint(model_params)
    model.get_bleu_score()
...
if __name__ == "__main__":
    ...
    if args.bleu: measure_model(args.model_params)
